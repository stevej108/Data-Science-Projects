...
def clean_cx():
    ...
    logger.info("Cleaning CX...")

    df = pd.read_csv(CX_FILE)
    ...
    for col in numeric_cols:
        cx[col] = clean_numeric(cx[col])

    # --------------------------------------------------
    # Quarter key
    # Useful for future joins
    # --------------------------------------------------

    cx["Quarter_Key"] = (
        cx["year"].astype(str)
        + "Q"
        + cx["quarter"].str.replace("Q", "", regex=False)
    )

    # --------------------------------------------------
    # Competition summary
    # Simple feature that is generally useful
    # --------------------------------------------------

    cx["Direct_Competitors"] = (
        cx["marshalls_in_same_center"].fillna(0)
        + cx["ross_stores_in_same_center"].fillna(0)
        + cx["t_j_maxx_in_same_center"].fillna(0)
    )
    ...
    for col in numeric_cols + ["Direct_Competitors"]:
        logger.info(
            f"{col:<30} {cx[col].dtype}"
        )
    ...
    logger.info(
        f"Unique Stores:           {cx['Store'].nunique():,}"
    )

    logger.info(
        f"Years:                   {cx['year'].nunique():,}"
    )

    logger.info(
        f"Quarter Keys:            {cx['Quarter_Key'].nunique():,}"
    )
    ...
    logger.info(
        f"\nDuplicate Store-Year-Quarter Keys: "
...
def clean_weather(
    ...
    logger.info(weather.columns.to_list())

    weather = (
        weather.loc[
            :,
            ~weather.columns.duplicated()
        ]
    )

    logger.info(
            "\nWeather Columns After De-Duplication"
        )
...
def aggregate_weather(
    ...
    logger.info(
        f"Monthly Weather Rows: "
        f"{len(monthly_weather):,}"
    )

    logger.info(monthly_weather.columns.to_list())
...
def generate_data_quality_report(spend, traffic, stores, cx):
    ...
    logger.info("")
    logger.info("CX Master")
    logger.info("-" * 60)

    

    logger.info(f"Total Stores:         {len(cx):>8}")
    
    logger.info(f"Distinct Grades:        {cx['facility_grade'].nunique():>8}")
...
def main():
    ...
    cx.to_parquet(
            INTERIM_DATA_DIR / "cx_clean.parquet",
            index=False
    )

    weather_monthly.to_parquet(
        INTERIM_DATA_DIR / "weather_monthly.parquet",
        index=False
    )
